...
1. Ensure PHP 8.1+ and MySQL 8+ are installed locally (the team currently runs MariaDB on port `3307`).
2. Copy `api/config/database.php` and `api/config/app.php` to environment-specific variants or set the required environment variables:

   ```bash
   # minimum variables for local development
   $env:DB_HOST="127.0.0.1"      # or your container host
   $env:DB_PORT="3307"          # default is 3306; override if MariaDB runs on 3307
   $env:DB_NAME="smarttutor"
   $env:DB_USER="root"
   $env:DB_PASS=""
...
4. Start the PHP development server from the project root (remember to export `DB_PORT` if you run on non-default 3306):
   ...
   # Bash example
   DB_PORT=3307 php -S localhost:8000 -t .
   ```
...
5. Available endpoints:
   ...
   - `GET /api/auth.php` – validate the current session token and retrieve the active user (requires `Authorization: Bearer {token}`)
   ...
   - `GET /api/security.php` and `GET /api/metrics.php` – admin-only dashboards guarded by JWT signature validation (supply a token for a user with `role=admin`)
...
> **JWT validation note:** `Security::validateJWT()` now powers the admin analytics endpoints, so keep your `JWT_SECRET` synchronized across environments (env var or `api/config/app.php`).
